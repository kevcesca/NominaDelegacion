...
export default function TablaCLCQuincenaTotales() {
    ...
    const fetchData = async () => {
        try {
            ...
            // Transformar datos para asegurar que cada fila tenga valores correctos
            const transformedData = json.map((item, index) => ({
                ...
                percepciones: item.percepciones || 0,
                deducciones: item.deducciones || 0,
                liquido: item.liquido || 0,
                EMPLEADOS: item.EMPLEADOS || 'N/A',
    ...
    const handleExport = (type) => {
        ...
        if (type === 'pdf') {
            ...
            saveAs(blob, 'consulta_clc_quincena_totales.csv');
        } else if (type === 'excel') {
            ...
            saveAs(
                ...
                'consulta_clc_quincena_totales.xlsx'
            );
        }
    };
    ...
    return (
        <Box display="flex" flexDirection="column" alignItems="center" padding="20px">
            <Typography variant="h4" gutterBottom>
                Consulta de Totales por Quincena
            </Typography>
